...
# ../../runNECRAnalysisEW.py LYSO-Proteus/ BGO/ BGO_BaF2_23.7/ BGO_EJ232_22.7/ LYSO-Proteus_BaF2_31.1/ LYSO-Proteus_EJ232_32.3/ > ./nec-output_all.txt
...
def analyse_all_folders(output_folders, **kwargs):
    ...
    grNECRToF.SetTitle(';activity (MBq);NECR (cps)')
    #grNECR.SetMinimum(0.)
    ...
    grNECRToF.Draw('APC')
    legendToF = ROOT.TLegend(0.45, 0.62, 0.99, 0.98)
    for index, output_folder in enumerate(output_folders, start=0):
        legendToF.AddEntry(grNECREW[index] ,get_legend_text(output_folder))
    legendToF.Draw('same')
    c1.Update()
    c1.Print(output_folder+"../necr_EWToF.pdf")
    r = True
    return r
